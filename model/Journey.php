...
class Journey extends DAO
{
                               ...
                               $cardNumber, $cardType, $cardExpiry)
    {
        ...
        if($newCustomer != -1) {
            if ($this->confirmQueue($journeyID, $newCustomer) === true) {
        ...
        } else echo "Customer was not created. Contact IT sector";
    }

    private function confirmQueue($journeyID, $customerID)
    {
        $table = "queue";
        $lastID = $this->getLastID($table, $this->idField) + 1;
        $sqlQuery = 'INSERT INTO ' . $table .
            ' VALUES(' . $lastID . ',' . $journeyID . ',' . $customerID . ',0,0)';
        $result = parent::query($sqlQuery) or die(parent::getConnection()->getConnection()->error);
        if ($result >= 0) {
            return true;
        } else return false;
